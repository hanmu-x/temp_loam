...
namespace loam
{
   ...
   // 接受激光扫描周期和最大迭代次数作为参数
   BasicLaserOdometry::BasicLaserOdometry(float scanPeriod, size_t maxIterations) : _scanPeriod(scanPeriod),
                                                                                    _systemInited(false),
                                                                                    _frameCount(0),
   ...
   // 将输入点 pi 根据当前的激光扫描周期 _scanPeriod 和与时间相关的强度信息进行处理，然后根据类 BasicLaserOdometry 中存储的位置和姿态变换信息 _transform，对输入点进行位置和姿态的变换，最终输出到 po 中。这通常用于激光雷达数据的运动畸变校正或者运动估计过程中的点云坐标转换
   void BasicLaserOdometry::transformToStart(const pcl::PointXYZI &pi, pcl::PointXYZI &po)
   {
      // s 计算为扫描周期的倒数乘以强度浮点数部分与整数部分之差，这个计算通常用于时间差分处理或类似的动态调整过程中
   ...
   size_t BasicLaserOdometry::transformToEnd(pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud)
   {
      ...
      for (size_t i = 0; i < cloudSize; i++)
      {
         ...
         // 计算出该点对应的实际时间比例,利用强度值的整数部分和小数部分来还原点的采集时间相对于扫描周期的位置
         float s = (1.f / _scanPeriod) * (point.intensity - int(point.intensity));
         // 根据时间比例 s 和当前帧的位姿变换 _transform，逆向调整点的位置（减去旋转和平移分量）是为了“回溯”点在扫描开始时的大致位置
         point.x -= s * _transform.pos.x();
         point.y -= s * _transform.pos.y();
         point.z -= s * _transform.pos.z();
         point.intensity = int(point.intensity);
         // 对点进行逆向旋转操作，先根据时间比例逆向旋转点云以大致回到起点位置，然后应用 _transform 的旋转逆向旋转，最后再根据IMU的初始和最终姿态进行旋转，以确保点云正确反映了从采集时刻到整个扫描周期结束时的完整位移和旋转变化
   ...
   // 将两个旋转操作的结果累积到一起，生成一个新的整体旋转表示。输入参数包括两组欧拉角 (cx, cy, cz) 和 (lx, ly, lz)，分别代表两个独立的旋转操作，以及输出参数 (ox, oy, oz) 用于存储累积后的旋转表示
   ...
   {
      // 计算出围绕最终x轴累积旋转后的新旋转角 ox srx 是计算 ox 时所需的一个中间表达式
